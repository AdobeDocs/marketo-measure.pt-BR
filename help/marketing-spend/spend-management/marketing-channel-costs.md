...
A variável [!DNL Marketo Measure] O recurso de gasto com marketing permite que os clientes façam upload de seus gastos em todos os canais, subcanais e campanhas. Quanto mais dados forem adicionados pelos clientes, mais os relatórios de ROI serão exibidos no Painel de atribuição de receita.
...
No nível mais granular, os clientes podem inserir gastos por Campanhas individuais, agrupadas em seu respectivo Canal. Para campanhas CRM, [!DNL Marketo Measure] O puxou a ID da campanha em uma coluna separada que ajuda a mapear o gasto offline da campanha do seu CRM para esta tabela. Adicionar gastos nesse nível permite que os clientes visualizem o ROI do Campaign e otimizem o desempenho pelo Campaign.
...
O intervalo de datas padrão começará a partir da data inicial com [!DNL Marketo Measure] até o mês atual. Para garantir que os custos permaneçam corretos, você não poderá inserir custos para meses futuros, mas poderá inserir custos para meses antes de sua parceria com a [!DNL Marketo Measure].
...
[!DNL Marketo Measure] O baixará automaticamente os custos do canal de marketing de plataformas com as quais ele tem uma integração de API, como Bing Ads e Google AdWords. Custos para plataformas que não estão integradas no [!DNL Marketo Measure] precisa ser carregado manualmente. Os canais de marketing devem ser configurados antes que os dados de custo sejam carregados.
...
Se estiver trabalhando no Google Sheets, baixe o arquivo primeiro. Não exclua ou exclua dados mensais, pois isso causa dificuldades ao tentar fazer upload do arquivo CSV para [!DNL Marketo Measure] posteriormente.
...
Se nenhum valor for inserido em um nível superior, como Canal ou Subcanal, [!DNL Marketo Measure] O somará automaticamente os níveis secundários para você, que serão apresentados quando o arquivo for carregado. Além disso, se a soma dos filhos for menor que um valor inserido para o pai, [!DNL Marketo Measure] adiciona uma linha &quot;Outros&quot; para mostrar a diferença no total.
...
Temos diferentes conjuntos de permissões na [!DNL Marketo Measure] Aplicativo. Para carregar um arquivo, você deve ser um &quot;AccountAdmin&quot;. Para contornar isso, solicite acesso ao seu AccountAdmin ou peça a ele que carregue o arquivo em seu nome. Uma lista de usuários e suas funções pode ser encontrada em **[!UICONTROL Minha conta]** > **[!UICONTROL Configurações]** > **[!UICONTROL Exibir/adicionar usuários da conta]**.
