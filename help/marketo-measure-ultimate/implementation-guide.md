...
ExL: visão geral dos conjuntos de dados: todos os dados assimilados com êxito na AEP são mantidos no Data Lake como conjuntos de dados. Um conjunto de dados é uma construção de armazenamento e gerenciamento para uma coleção de dados, normalmente uma tabela, que contém um esquema (colunas) e campos (linhas).

## Criação de um esquema {#creating-a-schema}

Recomendamos que os clientes usem um utilitário de geração automática para criar 10 esquemas B2B padrão.

As etapas para baixar e configurar o utilitário podem ser encontradas na seção &quot;Configurar namespaces B2B e utilitário de geração automática de esquema&quot; no ExL: namespaces B2B e esquemas

Para clientes com um direito de CDP: crie esquemas na página Origens
...
ExL: visão geral dos fluxos de dados
...
ExL: requisito de integridade de dados do Marketo Measure Ultimate
...
Monitoramento: Fontes → Página Fluxos de dados para verificar o status dos fluxos de dados Para exibir os detalhes da atividade de um conjunto de dados, basta clicar no conjunto de dados Para exibir erros de fluxo de dados, selecione um fluxo de dados, escolha uma execução e clique em &quot;Visualização de diagnóstico de erro&quot;

## Inspeção de dados {#data-inspection}

ExL: requisito de integridade de dados do Marketo Measure Ultimate Este documento inclui campos obrigatórios para cada XDM, bem como consultas de inspeção. Ele será publicado no ExL.

Opção 1: para executar consultas diretamente da interface do usuário, acesse a guia Consultas em Gerenciamento de dados.

![](assets/marketo-measure-ultimate-implementation-guide-.png)

Opção 2: Baixar e usar PSQL (mais rápido e confiável) ExL: Conectar PSQL ao Serviço de consulta
...
Itens a serem observados: os dados de uma determinada entidade (por exemplo, Conta) de uma determinada fonte só podem entrar em um conjunto de dados. Cada conjunto de dados pode ser incluído somente em um fluxo de dados. As violações interromperão o fluxo de dados no tempo de execução.
...
Na Medida, a moeda padrão precisa ser definida na seção &quot;Moeda&quot;. Se os clientes usarem várias moedas, o esquema de taxa de conversão monetária deverá ser preenchido na AEP para que possamos ler e usar para conversões.

Mapeamento de estágios Não importamos estágios automaticamente dos dados do cliente, portanto, todos os estágios devem ser mapeados manualmente.
...
Regras de membros da campanha Precisam escolher um conjunto de dados e definir regras para cada um.

Regras de eventos de experiência Precisam escolher um conjunto de dados e selecionar tipos de atividade.
...
Canais offline Não fazemos regras de mapeamento de canal específicas do conjunto de dados, portanto, isso seria global.
