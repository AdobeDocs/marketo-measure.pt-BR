...
Configurar o aplicativo do [!DNL Marketo Measure] no provedor de identidade. Consulte a documentação externa listada abaixo para obter instruções passo a passo.
    ...
    b. Quando solicitado a fornecer a URL de Restrição de Público-Alvo ou o identificador exclusivo definido pelo aplicativo, use [https://BizibleLPM](https://biziblelpm/)

Alterar para SSO personalizado no aplicativo do [!DNL Marketo Measure]

    a. Depois que o Grupo de Cobrança tiver sido habilitado para sua Conta, você poderá navegar para [!UICONTROL Configurações] >>[!UICONTROL Segurança] >> [!UICONTROL Autenticação]
...
Preencha as configurações de conexão do provedor de identidade
    ...
    a. O provedor de identidade pode fornecer um documento .xml de metadados IdP que obterá os campos de configuração necessários. Carregue o conteúdo do documento .xml ou preencha os três campos abaixo com base nas informações obtidas durante o processo de configuração do provedor de identidade. **Não é necessário concluir ambos.**
    
    i. URL IdP: O URL que [!DNL Marketo Measure] precisa apontar para para autenticar seus usuários no aplicativo [!DNL Marketo Measure] . Às vezes chamado de &quot;URL de redirecionamento&quot;.
    ii. Emissor de IdP: um identificador exclusivo do provedor de identidade. Às vezes chamada de &quot;Chave externa&quot;.
...
Defina e mapeie suas configurações de Atributo do usuário para o respectivo nome, sobrenome e endereço de email.
    ...
    a. Ao inserir os atributos SAML, o  [!DNL Marketo Measure]  poderá reconhecer seus usuários pelas informações transmitidas.
...
Defina e mapeie suas configurações de função de usuário para as respectivas funções ou grupos classificados pelo seu IdP.
    ...
    a. Os clientes têm a opção de atribuir  [!DNL Marketo Measure] funções de usuário com base em grupos definidos em seu Provedor de identidade. Ao inserir os atributos SAML, o  [!DNL Marketo Measure]  poderá mapear as funções e grupos do seu usuário para as permissões do  [!DNL Marketo Measure] . É altamente recomendável que você configure essas funções para que o  [!DNL Marketo Measure] administrador tenha direitos suficientes para atualizar sua conta.
    
    b. Se nenhuma função ou grupo for mapeado, a configuração padrão será que todos os funcionários no Provedor de Identidade terão acesso de usuário Padrão.
    
    i. [!DNL Marketo Measure] Usuário Padrão: Forneça o valor da função ou do grupo (do seu provedor SSO) para usuários que devem ter acesso somente leitura ao  [!DNL Marketo Measure] aplicativo.
    ii. [!DNL Marketo Measure] Usuário Administrador da Conta: forneça o valor da função ou do grupo (do seu provedor SSO) para usuários que devem ter acesso administrativo ao  [!DNL Marketo Measure] aplicativo. Isso significa que a função tem acesso para alterar configurações e definições relacionadas à sua Conta.
...
Testar a configuração de logon único
    ...
    a. Antes de clicar em Salvar, você deverá clicar no botão [!UICONTROL Testar Autenticação SAML] para verificar se suas configurações foram definidas corretamente.
...
Salve as configurações e oriente os(as) colegas a utilizar o [!UICONTROL logon único] com o novo URL de logon personalizado.
    ...
    a. Importante: depois de salvar as novas configurações de Autenticação, é possível que a sessão termine quando você navegar para uma nova página porque você desabilitou o logon dos Usuários do CRM e habilitou o SSO Personalizado.

![a. Importante: depois que você Salvar as novas configurações de Autenticação, será possível](assets/discover-control-3.png)

Experimente.

    a. Use sua nova URL de Entrada personalizada e tente fazer logon novamente no  [!DNL Marketo Measure] Aplicativo com suas credenciais de Provedor de Identidade.
...
* [Ative Diretory](https://docs.microsoft.com/pt-BR/azure/active-directory/active-directory-saas-custom-apps)
